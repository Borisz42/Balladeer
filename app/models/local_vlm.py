    ...
    def generate_story_and_lyrics(
        ...
        if is_instrumental:
            system_prompt = (
                ...
                "   - 6s to 12s Section: 15 to 25 words (1-2 complete sentences)\n"
                "   - 14s to 24s Section: 35 to 55 words (2 to 4 detailed narrative sentences exploring the sights, streets, architecture, and atmosphere)\n"
                ...
                "The morning air is filled with excitement as new perspectives open up around every turn.\n\n"
                "[0:24-0:28] [Outro] (4s)\n"
                "As the day comes to a close, we hold onto memories from an unforgettable journey."
            )
            prompt = (
                f"INSTRUCTIONS:\n"
                f"Write timed spoken documentary narrative voiceover subtitles (~2.2 words per second speaking tempo).\n"
                f"- For 4s Intro / Outro: EXACTLY ONE short sentence of 6 to 9 words.\n"
                f"- For 14s to 24s sections: You MUST write 2 to 4 detailed descriptive sentences (35 to 55 words total) telling the story of the destination and sights seen.\n\n"
        ...
        system_prompt = (
            ...
            "   - Line 1 rhymes with Line 2 (different rhyming words: e.g. light/bright, air/share).\n"
            "   - Line 3 rhymes with Line 4 (different rhyming words: e.g. sky/high, wall/fall).\n"
            "   - For sections >= 14s (e.g. 16s-24s), you MUST write EXACTLY 4 full rhyming lines (AABB).\n"
            "   - For sections < 14s (e.g. 6s-12s), you MUST write EXACTLY 2 full rhyming lines (AA).\n"
            "   - NEVER repeat the same word at the end of both rhyming lines (e.g. do NOT rhyme meet with meet).\n"
            "2. Weave in specific real landmarks, destinations, and sights from the travel story (e.g. Le Pré-Saint-Gervais, morning light, towers, murals, street cafes).\n"
            "3. NEVER copy literal camera OCR phrases (e.g. NEVER mention 'white card', 'qr code', 'metal detector', 'holds a'). Transform them into poetic travel imagery.\n"
            "4. For [Intro] and [Outro] instrumental sections, output ONLY: [Instrumental - Acoustic guitar description]. Do NOT write singing lines under Intro/Outro.\n"
            ...
            "Past the towers on the skyline rising tall\n"
            "Finding beauty in the murals on the wall\n\n"
            "[0:24-0:28] [Outro] (4s)\n"
            "[Instrumental - Warm acoustic fade-out]"
        )
        prompt = (
            f"INSTRUCTIONS:\n"
            f"Write poetic rhyming song lyrics with strict AABB rhyme scheme for each section.\n"
            f"- For sections >= 14s, write EXACTLY 4 rhyming lines (AABB: 2 distinct rhyming pairs). "
            f"Every rhyming pair MUST end on different words!\n"
            f"- Mention the actual destination, atmosphere, and sights from the travel story.\n"
            f"- For [Intro] and [Outro], output ONLY the instrumental bracket tag.\n\n"
