  ...
  return (
    <div className="glass-panel rounded-2xl p-5 border border-slate-800 space-y-5">
      {audioTrack && (
        <audio
          ref={audioRef}
          src={`http://localhost:8000/api/projects/${project.id}/audio/master`}
          onTimeUpdate={handleTimeUpdate}
          onEnded={() => setIsPlaying(false)}
        />
      )}

      {/* Top Grid: Canvas Live Preview & Control Bar */}
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
        ...
        <div className="lg:col-span-7 flex flex-col justify-between space-y-3 bg-slate-950/60 rounded-xl p-4 border border-slate-800/80">
          <div>
            <div className="flex items-center justify-between pb-2 mb-2 border-b border-slate-800">
              <span className="text-xs font-bold text-slate-300 uppercase tracking-wider">
                Montage Studio Controller
              </span>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-mono">
              <div className="bg-slate-900 p-2 rounded border border-slate-800">
                <span className="text-slate-400 block text-[10px]">BPM</span>
                <strong className="text-teal-300">{audioTrack?.bpm || 120}</strong>
              </div>
              <div className="bg-slate-900 p-2 rounded border border-slate-800">
                <span className="text-slate-400 block text-[10px]">Cuts / Slices</span>
                <strong className="text-cyan-300">{slices.length}</strong>
              </div>
              <div className="bg-slate-900 p-2 rounded border border-slate-800">
                <span className="text-slate-400 block text-[10px]">Aspect</span>
                <strong className="text-white">{aspect}</strong>
              </div>
              <div className="bg-slate-900 p-2 rounded border border-slate-800">
                <span className="text-slate-400 block text-[10px]">Mode</span>
                <strong className="text-amber-300">
                  {audioTrack?.is_instrumental ? 'Instrumental' : 'Vocal Sync'}
                </strong>
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-800">
            <button
              onClick={togglePlay}
              disabled={!audioTrack}
              className="w-9 h-9 rounded-lg bg-teal-500 hover:bg-teal-400 disabled:opacity-50 text-slate-950 flex items-center justify-center font-bold shadow-md shadow-teal-500/20 transition"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 px-3 py-2 rounded-lg text-xs font-semibold transition"
              title="Split active slice at playhead position"
            >
            ...
            <button
              ...
              className="flex items-center gap-1 bg-slate-800 hover:bg-slate-700 disabled:opacity-50 text-slate-200 border border-slate-700 px-3 py-2 rounded-lg text-xs font-semibold transition"
            >
              <RotateCcw className="w-3.5 h-3.5 text-teal-400" />
              {isSolving ? 'Solving...' : 'Auto-Solve Cuts'}
            </button>
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-gradient-to-r from-teal-500 to-cyan-500 hover:from-teal-400 hover:to-cyan-400 disabled:opacity-50 text-slate-950 px-4 py-2 rounded-lg text-xs font-bold transition shadow-lg shadow-teal-500/20"
            >
              <Film className="w-3.5 h-3.5" />
              {isRendering ? 'Rendering NVENC...' : 'Export Final MP4'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs text-slate-400 font-mono">
          ...
          <div className="flex items-center gap-1 bg-slate-950 border border-slate-800 rounded px-1.5 py-0.5">
            <button onClick={() => setZoom((z) => Math.max(30, z - 15))} className="p-0.5 hover:text-white">
              <ZoomOut className="w-3 h-3" />
            </button>
            <span className="text-[10px] px-1">{zoom}px/s</span>
            <button onClick={() => setZoom((z) => Math.min(150, z + 15))} className="p-0.5 hover:text-white">
        ...
        <div
          ref={timelineRef}
          onClick={handleSeek}
          className="timeline-grid overflow-x-auto relative rounded-xl bg-slate-950/80 border border-slate-800 cursor-pointer select-none pb-2"
          style={{ minHeight: '320px' }}
        >
          <div style={{ width: `${Math.max(totalWidth, 800)}px` }} className="relative">
            {/* Playhead Marker */}
            <div
            ...
            {/* Track 1: Beat Grid */}
            <div className="h-8 border-b border-slate-800/80 bg-slate-900/90 flex items-center relative font-mono text-[10px]">
            ...
            <div className="h-9 border-b border-slate-800/60 bg-slate-900/50 relative flex items-center">
              {audioTrack?.aligned_lyrics?.map((wordObj, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className={`absolute h-6 rounded px-1 flex items-center justify-center text-[11px] font-semibold transition-all ${
                      isActive
            ...
            <div className="h-52 relative bg-slate-950/60 p-2">
              {slices.map((slice, idx) => {
                ...
                return (
                  <div
                    ...
                    className={`absolute top-6 bottom-3 rounded-xl overflow-hidden border-2 transition-all group cursor-pointer flex flex-col justify-between p-2 ${
                      isCurrent
                        ? 'border-cyan-400 bg-cyan-950/40 shadow-xl z-20 scale-[1.01]'
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center justify-between gap-1 text-[10px] text-slate-400 font-mono" onClick={(e) => e.stopPropagation()}>
                        ...
                        <div className="flex items-center gap-0.5">
                          <button
                            ...
                            disabled={idx === 0}
                            className="w-4 h-4 rounded bg-slate-800 hover:bg-slate-700 text-slate-300 disabled:opacity-30 flex items-center justify-center"
                          >
                    ...
                    <div className="text-[9px] text-teal-400 font-medium opacity-0 group-hover:opacity-100 transition text-center flex items-center justify-center gap-0.5">
                      <Sparkles className="w-2.5 h-2.5" /> Swap Shot
                    </div>
                  </div>
                );
              })}
